...
float pulse_speed;
/*----------------------------------------------------------------------------------------------------------------*/
void PWM_Configuration(int revolution);
...
void PWM_Configuration(int desired_rpm)
{
	// Cap clock
	...
	/*Configure Frequency for PWM by: ARR=TIM_CLK/(UEV.(PSC+1).(RCR+1))=227 Hz in which UEV=44Hz, PSC=7200, RCR=1 */
	TIM_TimeBaseStructure.TIM_ClockDivision=TIM_CKD_DIV1;					// TIM_CKD_DIV1((uint16_t)0x0000)
	TIM_TimeBaseStructure.TIM_CounterMode=TIM_CounterMode_Up;
	TIM_TimeBaseStructure.TIM_Period=227-1;
	...
	/*Pulse calculated from speed
	Do phan giai encoder la x  vong/phut Ex: 11 xung/ vong	
	*/
	pulse_speed=(227/240)*desired_rpm;
	TIM_OCInitStructure.TIM_Pulse=pulse_speed;															
