...
volatile uint16_t encoder_cnt, encoder_cnt_pre;
//volatile uint16_t rate, actual_speed;
...
{ 	
	...
	//Dung timer4 lam bo dinh thi 10ms, sampling time--------------
	Encoder_Reading_Configuration();
  NVIC_Configuration();
	
	//dung timer2 lam PWM generator, Period=8000 
	//Run_Motor(3000);
	//Stop_Motor_no_BRA();
